...
if __name__ == '__main__':
    # 加载模型
    # model = RTDETR(r'ultralytics/cfg/models/rt-detr/rtdetr-l.yaml')  # 不使用预训练权重训练
    model = RTDETR(r'F:/ultralytics/ultralytics/cfg/models/rt-detr/rtdetr-l-gda.yaml').load("rtdetr-l.pt")  # 使用预训练权重训练
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data=r'F:/ultralytics/ultralytics/cfg/datasets/mydata3.yaml',
        epochs=300,  # (int) 训练的周期数
        patience=100,  # (int) 等待无明显改善以进行早期停止的周期数
        batch=2,  # (int) 每批次的图像数量（-1 为自动批处理）
        ...
        optimizer='AdamW',  # (str) 要使用的优化器，选择=[SGD，Adam，Adamax，AdamW，NAdam，RAdam，RMSProp，auto]
        ...
        close_mosaic=20,  # (int) 在最后几个周期禁用马赛克增强
        ...
        # 超参数 ----------------------------------------------------------------------------------------------
        lr0=0.0001,  # (float) 初始学习率（例如，SGD=1E-2，Adam=1E-3）
        lrf=0.001,  # (float) 最终学习率（lr0 * lrf）
        momentum=0.900,  # (float) SGD动量/Adam beta1
        weight_decay=0.0001,  # (float) 优化器权重衰减 5e-4
